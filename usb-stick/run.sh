...
# === Shim deploy + bind-mount BEFORE Bose's init starts SoftwareUpdate ===
#
# Live-verified 2026-05-28 evening: Bose's init exec's
# /opt/Bose/SoftwareUpdate at ~uptime=32s on a cold boot. Our run.sh
# initial_snapshot runs at uptime=17s. So this block runs in the
# ~15 s window BEFORE Bose's init touches SoftwareUpdate, which is
# exactly what bind-mount needs.
#
# The wrapper file we mount is a tiny shell script that exec's the
# REAL Bose binary (snapshotted to $PERSIST/lib/SoftwareUpdate-real)
# with LD_PRELOAD set. Bose's init then exec's our wrapper instead
# of the real binary; the kernel reads the `#!/bin/sh` and runs the
# wrapper; the wrapper exec's the real binary with LD_PRELOAD. The
# very first SoftwareUpdate process Bose registers in its IPC mesh
# has the STR shim active from the start. No kill, no mesh
# disruption — that was the failure mode of the older watchdog
# approach (#27 in development notes).
sync_shim_to_nand() {
...
setup_shim_bindmount() {
    if [ -e "$SHIM_DISABLE" ]; then
        log "shim bind-mount: disabled via $SHIM_DISABLE marker"
        return 0
    fi
    if [ ! -r "$NAND_SHIM" ]; then
        log "shim bind-mount: no shim at $NAND_SHIM, skip"
        return 0
    fi
    if [ ! -e /opt/Bose/SoftwareUpdate ]; then
        log "shim bind-mount: /opt/Bose/SoftwareUpdate absent, skip (non-SoftwareUpdate variant)"
        return 0
    fi
    if mount 2>/dev/null | grep -q " /opt/Bose/SoftwareUpdate "; then
        log "shim bind-mount: already active"
        return 0
    fi
    if cp /opt/Bose/SoftwareUpdate "$SHIM_BACKUP.new" 2>/dev/null \
       && mv "$SHIM_BACKUP.new" "$SHIM_BACKUP" 2>/dev/null; then
        ...
        log "shim bind-mount: cp /opt/Bose/SoftwareUpdate -> $SHIM_BACKUP failed"
...
# Bose's init exec's /opt/Bose/SoftwareUpdate; that path is
# bind-mounted to this wrapper. The wrapper exec's the real
# binary (kept untouched at SHIM_BACKUP) with LD_PRELOAD set
# so STR's accept() shim hooks port 17008 from the very first
# process instance, preserving Bose's IPC mesh registration.
exec env LD_PRELOAD=/mnt/nv/streborn/lib/str-shim.so /mnt/nv/streborn/lib/SoftwareUpdate-real "$@"
WRAP_EOF
    ...
    if mount --bind "$SHIM_WRAPPER" /opt/Bose/SoftwareUpdate 2>/dev/null; then
        log "shim bind-mount: $SHIM_WRAPPER -> /opt/Bose/SoftwareUpdate active at uptime=$(uptime_s)s"
    else
        log "shim bind-mount: mount --bind failed at uptime=$(uptime_s)s"
    fi
}
setup_shim_bindmount
...
# bind-mount on the next boot.
SHIM_DISABLE="$PERSIST/state/shim-disable"
SHIM_BACKUP="$PERSIST/lib/SoftwareUpdate-real"
SHIM_WRAPPER="$PERSIST/lib/SU-wrapper.sh"
setup_shim_bindmount() {
    if [ -e "$SHIM_DISABLE" ]; then
        setup_log "shim bind-mount: disabled via $SHIM_DISABLE marker"
        return 0
    fi
    if [ ! -r "$NAND_SHIM" ]; then
        setup_log "shim bind-mount: no shim at $NAND_SHIM, skip"
        return 0
    fi
    if [ ! -e /opt/Bose/SoftwareUpdate ]; then
        setup_log "shim bind-mount: /opt/Bose/SoftwareUpdate absent, skip (non-SoftwareUpdate variant)"
        return 0
    fi
    # Idempotent: if a previous run already bind-mounted, skip.
    # mount --bind is RAM-based and lost on reboot, so on a fresh
    # boot we always start without an active mount.
    if mount 2>/dev/null | grep -q " /opt/Bose/SoftwareUpdate "; then
        setup_log "shim bind-mount: already active"
        return 0
    fi
    # Snapshot the real binary to NAND so the wrapper can exec it.
    # On the very first install /opt/Bose/SoftwareUpdate IS the
    # rootfs binary; subsequent boots the snapshot is already there
    # but re-copying does not hurt — the file content does not
    # change between Bose firmware revisions on a given box.
    if cp /opt/Bose/SoftwareUpdate "$SHIM_BACKUP.new" 2>/dev/null \
       && mv "$SHIM_BACKUP.new" "$SHIM_BACKUP" 2>/dev/null; then
        chmod +x "$SHIM_BACKUP"
    else
        setup_log "shim bind-mount: cp /opt/Bose/SoftwareUpdate -> $SHIM_BACKUP failed"
        rm -f "$SHIM_BACKUP.new" 2>/dev/null
        return 1
    fi
    # Write wrapper. The kernel reads `#!/bin/sh` from the first
    # two bytes and runs /bin/sh against the script with Bose's
    # original argv preserved via "$@".
    cat > "$SHIM_WRAPPER.new" <<'WRAP_EOF'
#!/bin/sh
# Auto-generated by STR's run.sh — do not edit directly.
# Bose's init exec's /opt/Bose/SoftwareUpdate; that path is
...
WRAP_EOF
    mv "$SHIM_WRAPPER.new" "$SHIM_WRAPPER" 2>/dev/null
    chmod +x "$SHIM_WRAPPER"
    # Bind-mount: kernel overlays the wrapper file on top of Bose's
    # rootfs path. The rootfs is read-only but bind-mounts onto it
    # are RAM-based and work even on read-only filesystems.
    if mount --bind "$SHIM_WRAPPER" /opt/Bose/SoftwareUpdate 2>/dev/null; then
        setup_log "shim bind-mount: $SHIM_WRAPPER -> /opt/Bose/SoftwareUpdate active at uptime=$(uptime_s)s"
    else
        setup_log "shim bind-mount: mount --bind failed at uptime=$(uptime_s)s"
    fi
}
setup_shim_bindmount
