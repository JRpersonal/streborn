...
| **Alarm clock** | `internal/alarm/`, scheduler in `internal/webui/alarms.go` | Agent endpoint `/api/alarms`, edited from the phone web UI | A preset, but timed: at a set time on set weekdays the agent wakes the speaker and recalls one of the six presets through the same path as `POST /api/play/<slot>`. The schedule runs on the box, because nothing else is awake at 06:30. The document lives in `/mnt/nv/streborn/alarms.json` (written only on a save) with an IANA zone from the embedded `time/tzdata`; what has already fired is kept in a separate `alarms-state.json` so a fire cannot clobber an edit. Nothing fires on a clock `clocksync` calls implausible, and nothing more than five minutes stale fires at all (see `docs/AUTOMATION.md`). |
