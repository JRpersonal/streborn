...
| **CineMate 120** | AM335x ARMv7l, module `sm2`, variant `lisa`, `variantMode=NoAP` | **Working (user-confirmed, full diagnostic)**: an owner installed STR over the network on 2026-09-10 and updated to v0.9.79 the day after, both clean, alongside his SoundTouch 20 (2026-09-12 bundle). Presets register natively, the Spotify engine is present, `boxHealth: ok`. The SoundTouch module here is the SoundTouch Wireless Adapter wired to the CineMate console over Bose Link, so the app talks to the adapter, not to the console; the console's own HDMI, ADAPTiQ and TV sound never depended on the cloud. This owner runs it on **Ethernet**, and STR treats a wired box properly since v0.9.80 (before that the Wi-Fi boot guard power-cycled the unused radio at every boot, costing about three and a half minutes of the startup). |
...
| **SoundTouch 520** | module `sm2`, variant `lisa` | **Working (user-confirmed)**: diagnostic in discussion #679 (2026-08-23) shows firmware 27.0.6 and both trust stores complete. Identical fingerprint to the CineMate 520, so from STR's side the two are the same chassis. |
