...
// renderUpdateNotes turns the curated release-notes Markdown (the
// changelog from the release manifest: "### Section" headings and
// "- item" bullets) into a safe minimal HTML subset. Everything is
// escaped; only headings and bullet lists are recognised, so a
// malformed or hostile payload can never inject markup. The top-level
// "## What's changed" heading is dropped (redundant with the banner).
function renderUpdateNotes(md) {
  ...
  for (const raw of lines) {
    ...
    if (line.startsWith('### ')) {
      closeList();
      html += `<div class="upd-notes-h">${escapeHtml(line.slice(4))}</div>`;
    } else if (line.startsWith('- ')) {
      if (!inList) { html += '<ul class="upd-notes-list">'; inList = true; }
      html += `<li>${escapeHtml(line.slice(2))}</li>`;
    } else {
      closeList();
      html += `<div class="upd-notes-p">${escapeHtml(line)}</div>`;
    }
  }
  closeList();
  return html;
...
async function checkAppUpdate() {
  ...
  try {
    ...
    if (!banner) return;
    // Show the curated "What's changed" notes from the release manifest
    // (feat/fix list generated by cmd/relnotes, served via the update
    // endpoint as m.notes) so the user can judge at a glance whether the
    // upgrade is worth it. Rendered through a safe minimal subset
    // (renderUpdateNotes), never as raw HTML.
    ...
    const notesHtml = notes ? renderUpdateNotes(notes) : '';
    banner.innerHTML = `
      <div><b>${escapeHtml(t('banner.appUpdateAvail'))}</b> ${escapeHtml(m.version)}</div>
      ${notesHtml ? `<details class="upd-notes"><summary>${escapeHtml(t('banner.whatsNew'))}</summary>${notesHtml}</details>` : ''}
